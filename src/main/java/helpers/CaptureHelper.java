...
public class CaptureHelper extends ScreenRecorder {
	...
	public static Rectangle browserPosition( WebDriver driver) {
		...
		if (screenName.equals("1")) {
			...
			driver.manage().window().setPosition(screenPositions.get(screenName));
			driver.manage().window().setSize(screenSizes.get(screenName));
			return new Rectangle(screenPositions.get(screenName).getX(), screenPositions.get(screenName).getY(),
					screenSizes.get(screenName).getWidth(), screenSizes.get(screenName).getHeight());
		}
	}
	...
	public static void startRecord(String methodName) throws IOException {
		...
		File file = new File(SystemHelper.getCurrentDir() + PropertiesHelper.getValue("RECORDED_PATH"));
		readFileConfig();
		Rectangle captureSize = browserPosition(DriverManager.getDriver());
